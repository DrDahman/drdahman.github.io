...
Requirements
------------
* Requires probably PmWiki 2beta44 and higher
* Last tested on PmWiki 2.1.26
...
function SectionEditMarkup($mainpage,$pagename, $text) {
	global $SectionEditWithoutHeaders, $SectionEditAutoDepth, $SectionEditActualPage;
	$out = '';
...
#	$p = preg_split('/((?<=header:\))|(?m:^)|(?:.*:\)))((?=!{1,'.$SectionEditAutoDepth.'}[^!])|(?=====))/', $text);
	$p = preg_split('/((?m:^)|(?<=:\)))((?=!{1,'.$SectionEditAutoDepth.'}[^!])|(?=====))/', $text);
	...
	for ($i = 0; $i < count($p); $i ++) {
		if (($i != 0) || ($pagename != $SectionEditActualPage)) {
			...
			$out .= "<:block>(:if auth edit:)(:sectionedit $pagename ".substr($p[$i],0,1)." $SectionEditActualPage:)(:ifend:)\n";
		}
		$out .= $p[$i];
	}
	return ($out);
...
if (($action == 'browse') | ($action == 'view')) {
	...
	Markup('editsecmarkgen', '<if', '/^.*(\(:nl:\)|header:\\)|\n)((!{1,6})|(====+)).*$/se', "SectionEditFirstTime(\$pagename,PSS('$0'))");
}
